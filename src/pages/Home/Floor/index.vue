...
export default {
  ...
  // 请求不能在floor内部去发，因为有多个floor，需要遍历出多个

  // 生命周期函数，挂载完成，
  // 第一次书写swiper的时候，在mounted当中书写是不可以的，但是为什么现在可以了？
  // 第一次书写轮播图的时候，是在当前组件内发的请求，但是当前组件没有发请求，而是在父组件内发的请求，且父组件已经完成了父子组件通信了，所以当前组件可以在mounted生命钩子里声明swiper
  /*
  // 计算属性
  computed: {
    ...mapState({

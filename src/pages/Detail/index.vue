            ...
            <ul class="goodsList">
              ...
              <li>
                <div class="list-wrap">
                  ...
                  <!-- 以前咱们的路由跳转，从A路由跳转到b路由，这里在加入购物车，进行路由跳转之前发请求
                  把购买的产品的信息通过请求的形式通知服务器存储 -->
                  <div class="operate">
...
export default {
  ...
  // 挂载完成
  mounted() {
    // 派发actions，并把路由中的skuId带过去
    this.$store.dispatch("getGoodesInfo", this.$route.params.skuId);
  },
  ...
  computed: {
    /* ...mapState({
      goodesInfo: (state) => {
        return state.detail.goodesInfo;
      },
    }), */
    ...
    // 给子组件的数据
    skuImageList() {
      // 如果服务器没有返回来的数据，skuInfo这个对象是个空对象，同样子组件里再用到数组里的对象时一样会碰到这样的问题所以里面加个空对象
  ...
  methods: {
    ...
    async addShopCar() {
      // 1：发请求把产品加入到服务器里
      // 当前这个地方怕发的action，但是需要判断请求成功或失败，进行相应的操作
      // this.$store.dispatch("addOrUpdateShopCart", 这行代码说白了就是在调用仓库中的addOrUpdateShopCart这个函数
      ...
      try {
        ...
        this.$router.push({
          name: "addcartsuccess",
          // 下面的这种手段进行路由传参可以，但是地址栏里很复杂
          // query: { skuInfo: this.skuInfo, skuNum: this.skuNum },
          // 一些简单的数据，通过query给路由组件传递过去，但是像产品信息的数据比较复杂通过会话存储(不持久化)会话结束就消失
          // 本地存储||会话存储，一般都只存储字符串
          query: { skuNum: this.skuNum },
        });
        // 我们采用的是会话存储 ---  转化成JSON字符串
